...
# Load metadata
metaData <- readRDS("../int/metaData_modules.rds")
...
pop_data <- non_meth_data %>%
  ...
  summarise(n=n(),
            ...
            n_event=sum(event),
            chol=paste0(round(median(chol, na.rm=T), 1), " (",
                        paste(round(iq_range(chol), 1), collapse="-"), ")"),
            ldl=paste0(round(median(ldl, na.rm=T), 1), " (",
                       paste(round(iq_range(ldl), 1), collapse="-"), ")"),
            hdl=paste0(round(median(hdl, na.rm=T), 1), " (",
                       paste(round(iq_range(hdl), 1), collapse="-"), ")"),
            tg=paste0(round(median(tg, na.rm=T), 1), " (",
                      paste(round(iq_range(tg), 1), collapse="-"), ")"),
            glu=paste0(round(median(glu, na.rm=T), 1), " (",
                       paste(round(iq_range(glu), 1), collapse="-"), ")"),
            hsCRP=paste0(round(median(hscrp, na.rm=T), 1), " (", 
                         paste(round(iq_range(hscrp), 1), collapse="-"), ")"),
            sbp=paste0(round(median(sbp, na.rm=T), 1), " (",
                       paste(round(iq_range(sbp), 1), collapse="-"), ")")) %>%
  setNames(c("Dataset", "Sample Size", "% female", "Age at blood draw", 
             "Mixed ancestry", "Body mass index (BMI)", "% smoke currently", "Smoking pack-years",
             "# prior CVD events", "# incident CVD events",
             "Total cholesterol (Chol; mg/dL)", "LDL-cholesterol (LDL; mg/dL)",
             "HDL-cholesterol (HDL; mg/dL)", "Triglycerides (TG; mg/dL)",
...
pretty_rfs <- c(bmi="BMI", smk_now="Smoking", smk_py="Smoking-PY",
                age="Age", chol="Chol", ldl="LDL", hdl="HDL", tg="TG", 
                hscrp="hsCRP", glu="Glu", sbp="SBP")
...
combp_res_tbl_pretty <- combp_res_tbl %>%
  mutate_at(c("p_region_slk", "p_adj_region_sidak", "p_region_slk_fhs"), 
            function(p) format(as.numeric(p), digits=3, scientific=T)) %>%
...
206 DMRs were found in WHI after Sidak multiple testing correction for each DMR based on its length. Of these, 3 were both found in FHS and replicated at a Bonferroni level (Table 3; Fig. 1). These regions were annotated to two cellular transport genes (SLC9A1 and SLC1A5) and TNRC6C, which codes for a scaffolding protein involved in miRNA-mediated translational repression. Of the three WGCNA modules identified above, brown4 CpG sites constituted part of 2 DMRs (at SLC9A1 & SLC1A5), while a single CpG from the blue module was also a member of the SLC9A1 DMR.

Of CpGs in these 3 identified DMRs, 4 (2 in SLC1A5 and 1 each in SLC9A1 and TNRC6C) were involved in methylation-quantitative trait loci (mQTL) based on the mQTLdb [@Gaunt2016]. For these 4 CpGs, a Mendelian randomization analysis was undertaken using the MR-Base platform [@Hemani2018] to assess the potential causality of their methylation for myocardial infarction (MI) and coronary heart disease CHD). While no associations were found for 3 of the 4 CpGs, a modest single-SNP relationship linked a cis-mQTL for cg22304262 with MI (p=0.013) and CHD (p=0.072). This SNP, rs8105903, was also found to modify SLC1A5 expression levels in whole blood based on the GTEx database [@Aguet2017]. Taken together, these data are consistent with a small effect of DNA methylation at SLC1A5 on CVD, possibly acting through regulation of SLC1A5 gene expression.
...
make_bed <- function(res_df) {
  res_df %>%
    ...
    mutate(start=as.numeric(pos), 
           ...
           negLogP=-log10(p)) %>%
    dplyr::select(chr, start, end, negLogP, CpG, z)
}
...
res_anno_region_fhs <- filter(res_anno_fhs, chr == chrom, 
                              start >= chromstart, end <= chromend)
slc9a1_plot_df <- bind_rows(list(res_anno_region_whi, res_anno_region_fhs), 
                     .id="Dataset") %>%
  mutate(Dataset=case_when(Dataset == 1 ~ "WHI", Dataset == 2 ~ "FHS"),
         Dataset=factor(Dataset, levels=unique(Dataset)))
slc9a1_plt <- ggplot(slc9a1_plot_df, aes(x=start / 1000, y=negLogP, 
                                  color=Dataset, shape=as.character(sign(z)))) +
  geom_point(size=5) +
  ...
  labs(x="Position on chr. 1 (kb)", y="-log(p)") +
...
res_anno_region_fhs <- filter(res_anno_fhs, chr == chrom, 
                              start >= chromstart, end <= chromend)
slc1a5_plot_df <- bind_rows(list(res_anno_region_whi, res_anno_region_fhs), 
                     .id="Dataset") %>%
  mutate(Dataset=case_when(Dataset == 1 ~ "WHI", Dataset == 2 ~ "FHS"),
         Dataset=factor(Dataset, levels=unique(Dataset)))
slc1a5_plt <- ggplot(slc1a5_plot_df, aes(x=start / 1000, y=negLogP, 
                                  color=Dataset, shape=as.character(sign(z)))) + 
  geom_point(size=5) +
  scale_shape_manual(values=c("-1"="-", "1"="+"), guide="none") +
  geom_vline(xintercept=dmr[1] / 1000, linetype="dashed") +
  geom_vline(xintercept=dmr[2] / 1000, linetype="dashed") +
  labs(x="Position on chr. 19 (kb)", y="-log(p)") +
...
res_anno_region_fhs <- filter(res_anno_fhs, chr == chrom, 
                              start >= chromstart, end <= chromend)
tnrc6c_plot_df <- bind_rows(list(res_anno_region_whi, res_anno_region_fhs), 
                     .id="Dataset") %>%
  mutate(Dataset=case_when(Dataset == 1 ~ "WHI", Dataset == 2 ~ "FHS"),
         Dataset=factor(Dataset, levels=unique(Dataset)))
tnrc6c_plt <- ggplot(tnrc6c_plot_df, aes(x=start / 1000, y=negLogP, 
                                  color=Dataset, shape=as.character(sign(z)))) + 
  geom_point(size=5) +
  scale_shape_manual(values=c("-1"="-", "1"="+"), guide="none") +
  geom_vline(xintercept=dmr[1]/1000, linetype="dashed") +
  geom_vline(xintercept=dmr[2]/1000, linetype="dashed") +
  labs(x="Position on chr. 17 (kb)", y="-log(p)") +
  ggtitle("TNRC6C locus")
```

```{r dmr-mr, include=F}
mr_res <- lapply(c("cg25130381", "cg22304262", "cg25607249", "cg07787614"), 
                 function(cpg) read_csv(paste0("../int/MR/", cpg, "_mr_results.csv")))
...
```{r monocyte-interactions, include=F}
run_cox_model_mono <- function(probe_data, covar_data, model_spec) {
  # Given a row of the M-value matrix, bind that methylation
  # data to the covariate data and run Cox proportional hazards regression
  # Args:
  #   probe_data: Methylation data for a specific CpG site
  ...
  my_try({
    cox_fit <- coxph(as.formula(model_spec), data=model_data, subset =! outlier_TF)
    c(CpG=CpG, summary(cox_fit)$coef[c('meth:Mono'), c('coef', 'z', 'Pr(>|z|)')])
  }, CpG)
}

monos_model_whi <- paste0("survObj ~ meth + age + race + bmi + smk_now + smk_py ",
                          "+ CD4T + CD8T + Bcell + NK + Mono + Gran + dnaPull + meth:Mono")

dmr_monos_res_whi <- lapply(all_rep_dmr_cpgs, function(dmr_cpgs) {
  whi_monos_res_list <- foreach(meth=iter(betas_whi[dmr_cpgs, ], by="row"), 
                                .packages="survival") %do%
    run_cox_model_mono(meth, nmd_whi, monos_model_whi)
  
  do.call(rbind, whi_monos_res_list) %>%
    data.frame(stringsAsFactors=F, check.names=F) %>%
    dplyr::rename(p=`Pr(>|z|)`) %>%
    mutate_at(c("coef", "z", "p"), as.numeric) %>%
    mutate(fdr=p.adjust(p, method="BH")) %>%
...
monos_model_fhs <- paste0("survObj ~ meth + sex + age + bmi + smk_now + smk_py", 
                        " + CD4T + CD8T + Bcell + NK + Mono + Gran + center + meth:Mono + ",
                        paste0("cpPC", 1:7, collapse="+"))

dmr_monos_res_fhs <- lapply(all_rep_dmr_cpgs, function(dmr_cpgs) {
  fhs_monos_res_list <- foreach(meth=iter(betas_fhs[dmr_cpgs, ], by="row"), 
                                .packages="survival") %do%
    run_cox_model_mono(meth, nmd_fhs, monos_model_fhs)
  
  do.call(rbind, fhs_monos_res_list) %>%
    data.frame(stringsAsFactors=F, check.names=F) %>%
    dplyr::rename(p=`Pr(>|z|)`) %>%
    mutate_at(c("coef", "z", "p"), as.numeric) %>%
    mutate(fdr=p.adjust(p, method="BH")) %>%
...
ct_interaction_model_fhs <- coxph(Surv(time, event) ~ fhs_ECs$brown4 * Mono + fhs_ECs$brown4 * CD4T + fhs_ECs$brown4 * CD8T + fhs_ECs$brown4 * Bcell + fhs_ECs$brown4 * NK + center + cpPC1 + cpPC2 + cpPC3 + cpPC4 + cpPC5 + cpPC6 + cpPC7, data=nmd_fhs) %>%
  broom::tidy() %>%
  filter(term == "fhs_ECs$brown4:Mono")
```
...
We observed the greatest enrichment of brown4 CpGs in 2 enhancer-associated chromatin annotations, DNase hypersensitivity sites (DHS) and H3K4me1 histone peaks, from monocytes compared to other blood cell subtypes (Fig. 2c). This could point towards monocyte-related biology and inflammatory processes as an important shared mechanism for cardiovascular risk between the two cohorts examined here. To validate this observation, an additional analysis based on the CellDMC method [@Zheng2018] was undertaken, based on the idea that cell type-specific epigenetic changes will result in statistical interactions between cell types and epigenetic quantities. Adapting this method to the epigenetic module level and applying it using partially-adjusted Cox models in FHS, no meaningful statistical interaction between monocyte fractions and brown module activation were observed (positive interaction; p=0.83). In contrast to the monocyte-related enrichments for the brown module, blue CpGs were enriched for DHS and promoter-associated H3K4me3 histone peaks from hematopoietic stem cells (HSCs), providing a link to the observed enrichment of development-related genes in this set.

```{r homer-prep}
homer_bed <- whi_anno_450k %>%
  select(CpG=Name, chr, pos) %>%
  mutate(score=1,
         strand="+",
         chromStart=pos - 1) %>%
  select(chrom=chr, chromStart, chromEnd=pos, name=CpG, score,
         strand)

homer_bed %>%
  filter(name %in% mod_cpgs$brown4) %>%
  write_tsv("../int/homer/brown4_cpgs_for_homer.bed")

homer_bed %>%
  filter(name %in% mod_cpgs$blue) %>%
  write_tsv("../int/homer/blue_cpgs_for_homer.bed")
```
  ...
  t(res) %>%
    ...
    mutate(Current=paste0(signif(beta_current, 2), 
           ...
           Cumulative=paste0(signif(beta_cum, 2), 
                             " (", signif(p_cum, 2), ")"),
           `Risk factor`=pretty_rfs[`Risk factor`]) %>%
...
kable(mod_cum_tbl, booktabs=T,
  ...
  footnote(general=c("* Regression results are presented as: beta (p-value).",
...
mod_rf_cvd_tbl <- cbind(format_mod_rf_cvd_table(rf_only[3:4, ]),
  ...
  mutate(`Risk factor`=pretty_rfs[`Risk factor`]) %>%
...
Data for the validation set came from a substudy of the Framingham Heart Study that measured DNA methylation in 2726 subjects from the Offspring Cohort. The Framingham Offspring Cohort was originally established in 1971 to follow 5209 descendants of the original Framingham Heart Study participants and their spouses [@Kannel1979]. Fasting blood samples for both methylation and clinical biochemistry were collected from participants at Exam 8, which took place from 2005-8. Blood samples were also provided for clinical biochemistry measurements in previous exams, constituting the "past exposures" examined here. Data are available in the dbGaP public repository (accession: phs000007.v29.p10; downloaded on September 27, 2017). Adjudicated cardiovascular event data was collected through 2015, and events were defined here as any of: MI, angina pectoris, stroke (approximately 90% being ischemic), or death from CHD (Framingham event codes 1-29). 

Blood-based biochemical markers (total cholesterol, low-density lipoprotein cholesterol (LDL), high-density lipoprotein cholesterol (HDL), triglycerides, glucose, hsCRP, and systolic blood pressure) were log10-transformed for all analyses. In addition, median imputation was used to fill missing values for BMI (20 individuals in total), medication use, and smoking status (thus assuming no medication use and no smoking where these values were missing). Diabetes was defined as either use of diabetes medication or a measured fasting blood glucose level of >125 mg/dL. While directly available in WHI, pack-years of smoking was approximated in FHS by multiplying the number of years since starting smoking by the current number of packs per day.
...
For the EWAS analysis, each CpG site was assessed using the same regression framework as in the module-based models, separately in both WHI and FHS. Methylation beta-values replaced eigenCpGs as the independent variable, and the full set of technical and biological covariates, including cell type composition estimates, was used. To remove the influence of beta-value outliers, samples were excluded for each CpG if their beta value was outside of the interval [25%ile - $3*IQR$, > 75%ile + $3*IQR$]. QQ plots and calculation of the genomic inflation factor $\lambda$ revealed that genomic inflation was not initially adequately controlled in FHS, but after additional adjustment for 7 CPACOR principal components (chosen based on a Scree plot assessment of CPACOR results), a reasonable inflation of $\lambda$ = 1.09 was achieved. CPACOR uses principal components analysis on the set of control probes from the methylation array in order to estimate and control for potential batch effects without disturbing biological signal [@Lehne2015]. Proportional hazards checks were implemented as in the module-based analysis for the top EWAS hits in WHI, and no systematic departure from the Cox regression assumptions were detected.

Comb-p, implemented as a Python module, was used to call differentially methylated regions (DMRs). The algorithm takes as input p-values from the EWAS, removing the requirement for additional covariate adjustment. Comb-p first calculates an autocorrelation function (ACF), for which a maximum distance of 1kb and a step size of 50 bases were used. Next, it uses the ACF to adjust each p-value using a Stouffer-Liptak-Kechris correction [@Kechris2010], followed by identification of contiguous regions of sites with adjusted p-values below some threshold (here, p<0.1 with no more than 500 bases between neighboring sites in a region). Finally, the ACF is recalculated out to the maximum region size (a step size of 50 was used here as well) and regional p-values are calculated using the Stouffer-Liptak test. For Sidak multiple testing correction of DMRs [@Sidak1967], Comb-p calculates the number of effective tests separately for each DMR as the number of loci tested divided by the number of loci in the region, thus approximating a correction for the total number of regions while accounting for region size.
...
Mendelian randomization (MR) analyses were undertaken for the 4 DMR CpGs that participated in mQTLs in whole blood in the mQTLdb [@Gaunt2016]. Relevant mQTL SNPs were retrieved from mQTLdb and used as input to the MR-Base platform [@Hemani2018]. MR analysis was run using MR-Egger, weighted median, inverse variance weighted, and weighted mode methods where possible, and the Wald ratio method where only one SNP was available. Outcome summary statistics for MI and CHD were taken from GWAS in the CARDIoGRAMplusC4D consortium [@Nikpay2015]. LD-based clumping was used to prune correlated SNPs prior to analysis, with default values used for all other parameters. For follow-up of one SNP of interest, tissue-specific eQTL data were explored in the GTEx Portal (release v7).
...
Cell type-module interaction analyses were undertaken using a similar approach to that of the CellDMC algorithm for cell type-specific differentially-methylated cytosine discovery [@Zheng2018]. Current implementations of CellDMC do not support time-to-event models. Partially-adjusted models were built in FHS as was done for the initial module-CVD replication, while including interaction terms between the brown4 module and each of 5 estimated cell type fractions (leaving out estimated granulocytes). The reported interaction term of interest was that between the brown4 module and monocyte fractions as influences time-to-event for CVD.
...
**FHS:** Framingham Heart Study Offspring Cohort
**Glu:** glucose
**HDL:** high-density lipoprotein cholesterol
**HSC:** hemtopoietic stem cell
**hsCRP:** high-sensitivity C-reactive protein
**LDL:** low-density lipoprotein cholesterol
**MI:** myocardial infarction
**SBP:** systolic blood pressure
**Smoking-PY:** pack-years of smoking 
...
This research was approved by the Tufts Health Sciences Campus Institutional Review Board (protocol 12592).
...
The main phenotype and genotype datasets are available from dbGaP: WHI (phs000200.v11.p3) and FHS (phs000007.v29.p10). Roadmap epigenomics data can be accessed at https://egg2.wustl.edu/roadmap/web_portal/. Code supporting the analyses described can be found at https://github.com/kwesterman/meth_cvd.
...
KW, DD, and JO conceived of the analysis. KW performed all analyses. PS, PJ, DD, and JO supervised the research project. KW drafted the manuscript. PS, PJ, SL, DD, and JO provided substantive review and edits to the manuscript. All authors read and approved the final manuscript.

## Acknowledgements

The Genotype-Tissue Expression (GTEx) Project was supported by the Common Fund of the Office of the Director of the National Institutes of Health, and by NCI, NHGRI, NHLBI, NIDA, NIMH, and NINDS. The data used for the analyses described in this manuscript were obtained from the GTEx Portal on 06/28/2019.
...
```{r risk-factor-correlation-plots, fig.cap="Risk factor-module relationships. (a) Pearson correlations between a series of traditional cardiovascular risk factors and module eigenCpGs (blue and brown4) are shown in each study population. (b) Pearson correlations between historical risk factor levels in FHS (across previous exams, x-axis) and current brown4 module activation are shown. Grey panels indicate that the risk factor in question was not available for the corresponding exam (LDL and hsCRP) or was not analyzed with respect to past exams (smoking and age)."}
current_corrs$module <- factor(current_corrs$module,
                               levels=c("brown4", "blue"),
                               labels=c("Brown", "Blue"))
current_corrs$rf <- factor(current_corrs$rf, 
                           labels=pretty_rfs[levels(current_corrs$rf)])
...
fhs_rf_corr_plt_data <- past_corrs %>%
  ...
  mutate(module="Brown",
         whichPast=stringr::str_to_title(whichPast),
         whichPast=factor(whichPast, 
                          levels=c(paste0("Exam", 1:7), 
                                   "Cumulative", "Current")))
...
```{r export}
beta_matrix_dims <- list(whi=dim(betas_whi),
                         fhs=dim(betas_fhs))

save(
  "non_meth_data", "pretty_rfs", 
  "beta_matrix_dims", "anno_450k",
  "wgcna_input_cpgs", "mod_cpgs", "mod_sizes",
  "fhs_mod_res_df", "whi_to_fhs_replication",
  "pop_data",
  "modTbl",
  "combp_res_tbl", "combp_res_tbl_pretty",
  "slc9a1_plot_df", "slc1a5_plot_df", "tnrc6c_plot_df",
  "gene_enrich_plots", "cpgi_enrich_plots", "epi_anno_plt_data",
  "current_corrs", "fhs_rf_corr_plt_data", "corr_limits",
  "horvath_overlap", "horvath_overlap_positive", "suz12_enrich_p",
  "mod_cum_tbl", "mod_rf_cvd_tbl",
  "ec_res", "nmd_fhs", "fhs_ECs", "fhs_res",
  "whi_mod_res_ccAdj_df", "whi_mod_res_allAdj_df", 
  "fhs_mod_res_df", "fhs_mod_res_allAdj_df", "fdr_tbl",
  "eforge_plt",
  "rf_corr_plt", "fhs_rf_corr_plt",
  file="../output/module_ewas_objects.RData"
)
```
