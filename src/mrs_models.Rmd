...
test_models <- function(meta, mrs, test_idx, testfunc) {
  ...
  map_dfr(covar_sets, function(cs) {
    ...
    testfunc(nmd_test, mrs_test, 1:nrow(nmd_test), covars=cs)
  }, .id="covariate_set")
}
...
calc_stack_weights <- function(stacked_df, studies) {
  ...
  # cox_fit <- coxph(as.formula(paste("Surv(time, event) ~",
  #                                  paste(studies, collapse=" + "))), 
  #                 data=stacked_df_filt)
  mrs_fit_cv <- cv.glmnet(as.matrix(stacked_df_filt[, studies]),
                          cvd_surv,
                          # stacked_df_filt$event,
                          family="cox",
                          # family="binomial",
                          # intercept=F,
...
# test_boots_csl <- function(meta, betas,
#                            boot_ssl_fits, index_df, 
#                            test_idx_sets, testfunc) {
#   # Loop through a series of resamples/folds and test held-out performance
#   boot_res_list <- lapply(
#     1:length(test_idx_sets), function (boot) {
#       test_idx <- test_idx_sets[[boot]]
#       ssl_preds <- meta
#       all_studies <- c("whi", "fhs_JHU", "fhs_UofMinn", "lbc36")
#       no_pred_studies <- c()
#       for (s in all_studies) {
#         fits_idx <- which(index_df$boot == boot & index_df$study == s)
#         if (all(boot_ssl_fits[[fits_idx]]$cpg_coefs == 0)) {
#           no_pred_studies <- c(s, no_pred_studies)
#         }
#         ssl_preds[[s]] <- calc_mrs_glmnet(
#           boot_ssl_fits[[fits_idx]]$cpg_coefs, betas_all)
#       }
#       pred_studies <- setdiff(all_studies, no_pred_studies)
#       csl_weights <- calc_stack_weights(ssl_preds[-test_idx, ],
#                                         pred_studies)
#       print(csl_weights)
#       if (length(no_pred_studies) > 0) csl_weights[no_pred_studies] <- 0
#       mrs <- csl_predict(ssl_preds, pred_studies,
#                          csl_weights[pred_studies])
#       test_models(meta, mrs, test_idx, testfunc)
#     })
#   bind_rows(boot_res_list, .id="Fold") %>%
#     mutate(covariate_set=factor(covariate_set,
#                                 levels=unique(covariate_set)))
...
# boots_csl_res <- test_boots_csl(nmd_all, betas_all,
#                                 boot_ssl_fits, 
#                                 boot_study_combos,
...
uofm_holdout_res_tbl <- test_models(
  ...
  nmd_all$batch == "fhs_UofMinn", test_mrs)

kable(uofm_holdout_res_tbl, caption="Results on held-out FHS subset")
```
...
surv_fit <- survfit(Surv(time, event) ~ nmd_roc$test_mrs>0,
                          data=nmd_roc)
# surv_fit <- survfit(Surv(time, event) ~ 1, data=nmd_roc)
plot(surv_fit, ymin=0.9, xmax=800, conf.int=T)
...
```{r ssl-bootstraps, include=F, cache=1}
...
stopCluster(cl)
```
```{r test-bootstrap-csl, include=F}
...
```{r cv-single-study-models, cache=1}
cl <- makePSOCKcluster(3)
registerDoParallel(cl)
...
cl <- makePSOCKcluster(5)
registerDoParallel(cl)
uofm_holdout_combined_model <- lapply(c("fhs_UofMinn", "fhs_JHU"),
                                   function(s) {
...
stopCluster(cl)
```
```{r test-combined-bootstraps, include=F}
...
stopCluster(cl)
```
```{r enet-cv-mvals, cache=1, cache.vars=c("cv_fits_mvals")}
...
stopCluster(cl)
```
```{r test-enet-cv, warning=F, include=F}
...
```{r full-combined-model, cache=1, cache.vars=c("final_mrs_fit")}
...
age_metric_enrich_Ps <- sapply(
  ...
  function(cpg_set) {
    ...
    2 * phyper(num_in_set, length(cpg_set), 
  ...
  full_res <- map_dfr(unique_levels, function(q) {
    res_per_study <- df %>%
      ...
      mutate(res=map(data, function(d) {
        ...
        tryCatch(test_mrs_with_SE(d, d$mrs, subset,
                                  basic_covars[basic_covars != "sex"]),
                 error=function(e) na_df)
      }))
...
grs_substudies <- list(
  grs_whi_as264=read_sscore("../int/whi_as264_grs.sscore"),
  grs_whi_garnet=read_sscore("../int/whi_garnet_grs.sscore"),
  grs_whi_gecco_cyto=read_sscore("../int/whi_gecco_cyto_grs.sscore"),
  grs_whi_gecco_init=read_sscore("../int/whi_gecco_init_grs.sscore"),
  grs_whi_hipfx=read_sscore("../int/whi_hipfx_grs.sscore"),
  # grs_whi_whims=read_sscore("../int/whi_whims_grs.sscore"),
  grs_whi_share_aa=read_sscore("../int/whi_share_aa_grs.sscore"),
  grs_whi_share_ha=read_sscore("../int/whi_share_ha_grs.sscore")
)
...
age_quantile_res <- strat_test_by_study(nmd_all, uofm_holdout_mrs, "age_group") %>%
  mutate(Group=factor(Group, levels=unique(Group)))
frs_quantile_res <- strat_test_by_study(nmd_all, uofm_holdout_mrs, "frs_group") %>%
  mutate(Group=factor(Group, levels=unique(Group)))
grs_quantile_res <- strat_test_by_study(nmd_all, uofm_holdout_mrs, "grs_group")
race_res <- strat_test_by_study(nmd_all, uofm_holdout_mrs, "race")
sex_res <- strat_test_by_study(nmd_all, uofm_holdout_mrs, "sex")
past_event_res <- strat_test_by_study(nmd_all, uofm_holdout_mrs, "pastEvent")
...
One example provides evidence that the systematic study differences are not solely due to batch effects:
The single-study model figure above suggests that an LBC36-trained score does not perform strongly/consistently in WHI or FHS. However, when the LBC36 score is tested on WHI and both FHS batches together, it performs much better (HR = `r lbc36_to_both$HR_per_SD`). This should not be due to batch effects, since *the LBC36 score has not "seen" either of the other datasets*. Indeed, it turns out that the LBC36-derived score is strongly higher in the WHI group as a whole, which has a much greater proportion of incident cases. Furthermore, this score is biologically plausible: the top-weighted CpG (cg06500161 near ABCG1) is already known to predict incident cardiometabolic disease based on prior investigations.
