...
```{r ewas-inflation, fig.show="hold", out.width="50%"}
...
```{r display-cpg-set-comparison, dependson=c("ewas-mrs","variance-thresholds","cvd-snp-mrs"), out.width="80%"}
...
```{r rf-mrs-distributions, fig.show="hold", out.width="50%"}
...
```{r evaluate-combined-models, out.width="50%"}
library(pROC)
...
The intent to this point was to determine the "maximum predictive power" of microarray-based methylation data in predicting CVD events. However, known associations of methylation with age, white blood cell proportions, etc. may explain a substantial fraction of this predictive power. 

```{r biological-significance}
...
singleCovs <- c("age","bmi","wbcPC1","wbcPC2","smk_now","ht_med","lipid_med","dm_med")
