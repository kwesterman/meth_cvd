...
```{r prereqs, include=F}
library(knitr)
opts_chunk$set(echo=F, fig.path="../doc/figures/ie2/", cache.path="../cache/incident_ewas-secondary/", fig.pos="h")
...
mp <- readRDS("../int/wgcna/modulePreservation.rds")
...
We first set out to find biologically relevant modules in an unsupervised manner using the WGCNA algorithm for `r nrow(betas_whi)` CpGs in WHI passing quality control filters. After weighted correlation network construction, topological overlap calculation, and subsequent clustering, `r length(modCpgs)` modules were uncovered, ranging in size from `r min(modSizes$modSize)` to `r sort(modSizes$modSize, decreasing=T)[2]` CpGs.
...
geneEnrichPlots <- plot_grid(brown4GeneGroupEnrichmentPlot + scale_y_continuous(limits=c(0,0.42)) + 
                               theme(legend.position="none") + labs(y="Brown4 CpG fractions"), 
                             blueGeneGroupEnrichmentPlot + scale_y_continuous(limits=c(0,0.42)) + 
                               theme(legend.title=element_blank()) + labs(y="Blue Cpg fractions"), 
                             ncol=2, rel_widths=c(2,3))
cpgiEnrichPlots <- plot_grid(brown4CpgIslandEnrichmentPlot + scale_y_continuous(limits=c(0,0.72)) + 
                               theme(legend.position="none") + labs(y="Brown4 CpG fractions"), 
                             blueCpgIslandEnrichmentPlot + scale_y_continuous(limits=c(0,0.72)) + 
                               theme(legend.title=element_blank()) + labs(y="Blue CpG fractions"), 
                             ncol=2, rel_widths=c(2,3))
...
ageMetricEnrichPs <- sapply(list(dnamAgeCpgs, phenoAgeCpgs, hannumCpgs), function(cpgSet) {
...
kable(modCumTbl, booktabs=T,
  ...
  footnote(number=c("Regression results are presented as: beta (p-value).",
                    "Models are adjusted for age, sex, smoking status and pack-years, estimated cell counts, study center, and 7 control probe principal components."))
...
topEwasRes <- fdrTbl %>%
  ...
  setNames(c("CpG","Chromosome","Direction of Association","P-value","Location","Annotated Gene","Replication P-value"))
...
```{r strat-analyses, warning=F, out.width="70%", fig.cap="Pearson correlations between CpGs in one dataset with FDR<0.25 and the corresponding CpGs in subsets of the other.", eval=F}
...
## Weighted correlation network provides context to EWAS findings


```{r wgcna-context, eval=F}
modSizes <- cpgToModule %>%
  group_by(module) %>%
  summarise(modSize=n())
whiResWgcna <- whiRes %>%
  inner_join(cpgToModule, by="CpG")
ewasModuleEnrichments <- whiResWgcna %>%
  filter(fdr<0.5) %>%
  group_by(module) %>%
  summarise(numHits=n()) %>%
  inner_join(modSizes, by="module") %>%
  mutate(obsOverExp=numHits/(sum(numHits)*modSize/nrow(whiRes))) %>%
  mutate(pEnrich=map2_dbl(numHits, modSize, function(n,m) phyper(n, m, nrow(whiRes)-m, sum(numHits), lower.tail=F))) %>%
  arrange(pEnrich) %>%
  mutate(pEnrichBonferroni=pEnrich*nrow(.),
         pEnrichBH=p.adjust(pEnrich, method="BH"))
topModules <- ewasModuleEnrichments$module[ewasModuleEnrichments$pEnrichBH<0.05]
```

We sought to add context to our results by placing CpGs in highly-correlated modules as found by WGCNA. WGCNA performed in WHI found `r length(modCpgs)` modules. To find modules most relevant to the EWAS results, we searched for enrichment of "possible hits" from EWAS using a relaxed threshold of FDR<0.5 (`r sum(whiRes$fdr<0.5)` CpGs in total). `r length(topModules)` of the modules were enriched for these CpGs (FDR<0.05; hypergeometric test). Further details on these modules, as well as summaries of their enriched Gene Ontology annotations, are below. Modules appeared to fall into two major categories: immune-related and development-related.

```{r module-rep-compare, message=F, eval=F}
### Does membership in a module mean better replication across to FHS?
compareDF <- whiResWgcna %>%
  mutate(inSig=module %in% topModules) %>%
  # mutate(inSig=module %in% c("brown","brown4","lavenderblush3")) %>%
  # mutate(inSig=module %in% c("brown4")) %>%
  inner_join(select(fhsRes, CpG, coef, p), by="CpG", suffix=c(".whi",".fhs"))
# repDF <- compareDF %>%
#   filter(fdr<0.5) %>%
...
repDF_byModule <- compareDF %>%
  mutate(module=ifelse(module %in% topModules, module, "allOtherModules")) %>%
  filter(p.whi<0.05) %>%
  mutate(replicated=(sign(coef.fhs)==sign(coef.whi) & p.fhs<0.05)) %>%
  group_by(module, replicated) %>%
  summarise(n=n()) %>%
  arrange(desc(replicated)) %>%
  right_join(expand.grid(module=topModules, replicated=c(T,F), stringsAsFactors=F)) %>%
  replace_na(list(n=0)) %>%
  summarise(frac=n[1]/sum(n),
            nMod=sum(n)) %>%
  mutate(moduleType=goEnrichmentStrings[module]) %>% 
  replace_na(list(moduleType="all other modules")) %>%
  arrange(moduleType) %>%
  mutate(module=factor(module, levels=module)) %>%
  arrange(desc(frac))

# fracIn <- repDF$n[1]/repDF$n[2]
# fracOut <- repDF$n[3]/repDF$n[4]
# inModChiSq.fit <- chisq.test(matrix(repDF$n, 2, 2, byrow=T))

numHits <- sum(whiRes$fdr<0.5)
numRep <- sum(repDF_byModule$frac * repDF_byModule$nMod)
...
#   summarise(n=n()) %>%
#   arrange(desc(replicated)) %>%
#   summarise(frac=n[1]/n[2]) %>%
#   replace_na(list(frac=0)) %>%
#   mutate(moduleType=case_when(
#     module %in% c("brown","brown4","lavenderblush3") ~ "Observed race interaction",
#     TRUE ~ "No race interaction"
#   )) %>%
#   arrange(moduleType) %>%
#   mutate(module=factor(module, levels=module))
# ggplot(repDF_byModule, aes(x=module, y=frac, fill=moduleType)) +
#   geom_bar(stat="identity") +
#   labs(y="Fraction nominally replicated") +
#   theme(axis.title.x=element_blank(), axis.text.x=element_text(angle=30))  
```

We then looked at the replication of our results in FHS in the context of these modules. None of the genome-wide hits replicated strongly in FHS. However, we looked more closely at the distribution across modules of possible hits (again, FDR<0.5 in WHI) that nominally replicated (uncorrected p<0.05 in FHS with same sign). Notably, sites from the brown4 module showed close to 50% replication, compared to less than 20% for all other modules.

```{r module-rep-compare-plot, eval=F}
ggplot(repDF_byModule, aes(x=module, y=frac, fill=moduleType)) +
  geom_bar(stat="identity") +
  labs(y="Fraction nominally replicated") +
  theme(axis.title.x=element_blank(), axis.text.x=element_text(angle=30))
...
More exploration to be done here for whether replication is better when there is no interaction among races in WHI or among sexes in FHS.

```{r interactions, warning=F, eval=F}
sexInteractions <- lapply(topModules, function(m) {
  cox.fit <- coxph(Surv(time,event)~center*mod, 
                   data=cbind(select(nmd_fhs, time, event, sex, center), mod=fhsECs[[m]]))
  pVal <- summary(cox.fit)$coef["centerUofMinn:mod",5]
  c(module=m, annot=goEnrichmentStrings[m], p=pVal)
})
a <- do.call(rbind, sexInteractions)

raceInteractions <- lapply(topModules, function(m) {
  cox.fit <- coxph(Surv(time,event)~race*mod+dnaPull, 
                   data=cbind(select(nmd_whi, time, event, race, dnaPull), mod=whiECs[[m]]))
  pVal <- summary(cox.fit)$coef["racewhite:mod",5]
  c(module=m, annot=goEnrichmentStrings[m], p=pVal)
})
a <- do.call(rbind, raceInteractions)

run_coxModel_sexInt <- function(probeData, covarData, model_spec) {
  # Given a row of the M-value matrix (corresponding to a CpG site), bind that methylation
  # data to the covariate data and run Cox proportional hazards regression
  CpG <- rownames(probeData)
  probeData <- as.vector(probeData)
  ...
  modelData <- cbind(covarData, meth=as.numeric(probeData),
                     survObj=Surv(time=covarData$time, event=covarData$event))
  myTry({
    cox.fit <- coxph(as.formula(paste0(model_spec, "+center:meth")), data=modelData, subset=!outlierTF)
    c(CpG=CpG, summary(cox.fit)$coef[c('meth:centerUofMinn'),c('coef','z','Pr(>|z|)')])
    # summary(cox.fit)$coef
  }, CpG)
}
sexInts <- foreach(meth=iter(betas_fhs[rownames(betas_fhs) %in% whiRes$CpG[1:10],], by="row"), 
        .packages="survival") %do%
  run_coxModel_sexInt(meth, nmd_fhs, ewasModel_fhs)

run_coxModel_raceInt <- function(probeData, covarData, model_spec) {
  # Given a row of the M-value matrix (corresponding to a CpG site), bind that methylation
  # data to the covariate data and run Cox proportional hazards regression
  CpG <- rownames(probeData)
  probeData <- as.vector(probeData)
  ...
  modelData <- cbind(covarData, meth=as.numeric(probeData),
                     survObj=Surv(time=covarData$time, event=covarData$event))
  myTry({
    cox.fit <- coxph(as.formula(paste0(model_spec, "+race:meth")), data=modelData, subset=!outlierTF)
    c(CpG=CpG, summary(cox.fit)$coef[c('meth:racewhite'),c('coef','z','Pr(>|z|)')])
  }, CpG)
}
raceInts <- foreach(meth=iter(betas_whi[rownames(betas_whi) %in% whiRes$CpG[whiRes$fdr<0.5],], by="row"), 
        .packages="survival") %do%
  run_coxModel_raceInt(meth, nmd_whi, ewasModel_whi)
raceIntPs <- do.call(rbind, raceInts)[,4] < 0.05
```
